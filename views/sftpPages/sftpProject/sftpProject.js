////views/sftpPages/sftpProject/sftpProject.js
// Global variables
...
async function loadFileList(path) {
  ...
  try {
    ...
    const response = await fetch(
      `/sftpFilesForProject?path=${encodeURIComponent(path)}`,
    );
    if (!response.ok) {
    ...
    // Filter out parent directory if we're at /project
    if (path === "/project") {
...
function renderFileList(files) {
  ...
  if (currentPath !== "/") {
    ...
    parentRow.querySelector(".parent-dir").addEventListener("click", (e) => {
      ...
      // Don't allow navigation above /project
      if (parentPath.startsWith("/project") || parentPath === "/project") {
  ...
  files.forEach((file) => {
    ...
    row.innerHTML = `
                    ...
                    <td class="file-actions">
                        ${
                          file.type === "file"
                            ? '<button class="btn btn-success btn-sm download-btn">Tải xuống</button>'
                            : ""
                        }
...
async function handleUpload(e) {
  ...
  try {
    const response = await fetch("/sftpUploadForProject", {
      method: "POST",
      body: formData,
    });
...
async function createFolder(e) {
  ...
  try {
    const response = await fetch("/sftpMkdirForProject", {
      method: "POST",
      headers: {
...
async function renameFile(e) {
  ...
  try {
    const response = await fetch("/sftpRenameForProject", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
async function deleteSelectedFiles() {
  ...
  try {
    const response = await fetch("/sftpDeleteForProject", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
async function downloadSelectedFiles() {
  ...
  try {
    ...
    if (selectedFiles.length === 1) {
      ...
      const response = await fetch("/sftpDownloadForProject", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
async function pasteFiles() {
  ...
  try {
    ...
    const response = await fetch("/sftpPasteForProject", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
